<template>
    <div class="ring-1 ring-gray-300 bg-white p-5 rounded-2xl shadow-sm" >
        ...
        <div  class="text-sm w-full gap-x-10 pt-8 space-y-8
                      ...
                      2xl:w-97
                      lg:w-97
                    " v-for="userEduc in userEducs" :key="userEduc">
        
        <span class="font-raleways font-bold grid grid-cols-1 "> 
        <p v-if="userEduc.schoolStatus==='Current'" class="text-gray-500">Studied at <strong class="text-black">{{userEduc.schoolName}}</strong></p>
        <p v-if="userEduc.schoolStatus==='Previous'" class="text-gray-500">Went to <strong class="text-black">{{userEduc.schoolName}}</strong></p>
         ...
         <div class=" ">
            ...
            ">
                <div class="flex flex-col space-y-2" id="schoolInput">
                  <input   type="text"  id="schoolName" v-model="schoolName" class="focus:outline-none rounded-xl h-10 pl-2 bg-transparent bg-gray-200"/>
                  <div class="flex items-center space-x-3 font-bold">
                    <input type="radio" v-model="status_ed" name="status_eds" value="Current" class="text-black"><p>Current</p>
                    <input type="radio" v-model="status_ed" name="status_eds" value="Previous"><p>Previous</p>
                  </div>
                 </div> 
            </div>
            ...
            ">
              <br>
              <span class="ml-2 uppercase font-bold text-sm text-gray-500">School</span> 
              <div v-for="schools in schoolsList" :key="schools" class="flex flex-col space-y-2" id="schoolInput">
                <input :id="schools.id" v-model="schools.school" class="focus:outline-none rounded-xl h-10 pl-2 bg-transparent bg-gray-200"/>
                <div class="flex items-center space-x-3 font-bold">
                    <input type="radio" :checked="schools.status==='Current'" :name="schools.id" v-model="schools.status" value="Current" class="text-black"><p>Current</p>
                    <input type="radio" :checked="schools.status==='Previous'" :name="schools.id" v-model="schools.status" value="Previous"><p>Previous</p>
    ...
    document.getElementsByName("status_eds").checked = false;
     ...
     save_data(){
       var info = {
        ...
        api.post('/api/updateEduc', info).then(()=>{
              console.log("infor", info)
              store.dispatch('getAuthEducation')
              VueSimpleAlert.alert("Schools created successfully", "Sucess","success")
